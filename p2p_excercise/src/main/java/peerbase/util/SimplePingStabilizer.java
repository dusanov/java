...
public class SimplePingStabilizer implements StabilizerInterface {
	...
	//	LoggerUtil.getLogger().fine(" ... stabilizer()... " + peer.getPeerKeys() );
		for (String pid : peer.getPeerKeys()) {
			
	//		LoggerUtil.getLogger().fine(" == stabilizing: " + pid);
			...
			try {
				...
				LoggerUtil.getLogger().fine(" PING ERROR " + e);
				todelete.add(pid);
			}
			if (isconn)
				peerconn.close();
		...
		for (String pid : todelete) {
			LoggerUtil.getLogger().fine(" ---- about to remove peer: " + pid);
			peer.removePeer(pid);
		}
	}
}
